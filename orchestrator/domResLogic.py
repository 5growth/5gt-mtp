...
def get_aggregate(r_zone):
    ...
    :return: 0 ok, -1 error
    """
    logger = logging.getLogger('cttc_mtp.domResLogic.get_aggregate')
    for zone in r_zone:
        vim = zone[0]
        pop = zone[1]
        r_zone_attr = zone[2]
...
def decompose_stitching_old(path):
    logger = logging.getLogger('cttc_mtp.domResLogic.decompose_stitching_old')
    # looking for vl in "direct" direction
    try:
        internal_path = []
        first = db_session.query(Dbstitching).filter_by(ingressElementId=path[0], ingressElementAddress=path[1]).first()
        while first.egressElementId != path[2] and first.egressElementAddress != path[3]:
            internal_path.append(first.egressElementAddress)
            internal_path.append(first.egressElementId)
            next = db_session.query(Dbstitching).filter_by(ingressElementId=first.egressElementId).first()
            internal_path.append(next.ingressElementAddress)
            first = next
        logger.info("Direct direction")
        return internal_path
    except AttributeError as e:
        logger.error("Something wrong in direct direction : '{}'".format(e))
    # looking for vl in "reverse" direction
    try:
        internal_path = []
        first = db_session.query(Dbstitching).filter_by(ingressElementId=path[2], ingressElementAddress=path[3]).first()
        while first.egressElementId != path[0] and first.egressElementAddress != path[1]:
            internal_path.append(first.egressElementAddress)
            internal_path.append(first.egressElementId)
            next = db_session.query(Dbstitching).filter_by(ingressElementId=first.egressElementId).first()
            internal_path.append(next.ingressElementAddress)
        ...
        logger.error("Something wrong in reverse direction : '{}'".format(e))
...
def decompose_wim(wim_id, g):
    ...
    topology = cop_connector.get_topology(result.id)
...
def decompose_stitching():
    ...
    :return: networkx graph
    """
    logger = logging.getLogger('cttc_mtp.domResLogic.decompose_stitching')
...
def reducing_graph(graph, first_name, second_name):
    ...
    :return: networkx graph
    """
    logger = logging.getLogger('cttc_mtp.domResLogic.reducing_graph')
    ...
    subgraph = graph.subgraph(nodes_involved)
    logger.debug(json_graph.node_link_data(subgraph))
    return subgraph
