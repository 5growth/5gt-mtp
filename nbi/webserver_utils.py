...
def resource_view(call_id=None):
    """
    Return the node-link format of the Resource view of MTP
...
def wim_resource_view(wim_id):
    """
    Return the node-link format of the Resource view for a specific WIM of MTP
    ...
    for ll in lls:
        ...
        if source_node != -1 and destination_node != -1:
            ...
            for i, o, e in g.edges(data=True):
                # if 'llid' in e and e['llid'] == ll.logical_links.logical_link_id:  # Old
                if 'llid' in e and e['llid'] == ll.logical_links.logical_link_id.split("_")[0]:
                    replicated = True
            if not replicated:
                g.add_edge(source_node,
                           destination_node,
                           # llid=ll.logical_links.logical_link_id, # old
